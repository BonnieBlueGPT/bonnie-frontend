...
import { useAnalyticsStore, useUserStore } from '@/store';
...
export const ConversionRewardSystem = ({ 
  ...
  useEffect(() => {
    ...
    trackEvent('conversion_reward_system_activated', {
      type,
      purchaseData,
  ...
  const handleRewardComplete = () => {
    if (rewardQueue.length > 0) {
      ...
      // Update user stats
      if (currentReward?.type === 'loyalty_points') {
        updateUserStats({
          loyaltyPoints: currentReward.data.newTotal,
